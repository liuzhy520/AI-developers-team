...
- Executors may only modify files listed in the assigned task card under `Allowed Paths`.
- Read only the paths listed in `Allowed Paths` plus any shared contract files explicitly referenced by the task card.
- Do not inspect or modify sibling task cards unless the task card marks them as dependencies.
...
- `acceptance` — acceptance criteria (checkboxes)
